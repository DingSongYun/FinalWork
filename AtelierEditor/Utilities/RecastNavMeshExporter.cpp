#include "RecastNavMeshExporter.h"
...
void RecastExporter::ExportNavigationData(const FRecastNavMeshGenerator* Generator, const FString& FileName)
{
#define APPEND_OFFMESHLINK 1

	const UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(Generator->GetWorld());
	...
	for (int32 Index = 0; Index < NavSys->NavDataSet.Num(); ++Index)
	{
		...
		if (NavData)
		{
			...
			#if APPEND_OFFMESHLINK
			struct FOffmeshLinkExportData
			{
				FVector Left;
				FVector Right;
				float Radius;
				int Direction;
				int Area = 5;	// constant value for server
				int Flags = 8;	// constant value for server
				int LinkId = 0;	// link id
				int Duration = 0.0f;	// time to pass the link on milliseceond
			};
			TArray<FOffmeshLinkExportData> OffmeshLinkDatas;

			TArray<AActor*> MySmartLinkProxy;
			UGameplayStatics::GetAllActorsOfClass(Generator->GetWorld(), ASmartNavLinkProxy::StaticClass(), MySmartLinkProxy);
			#endif

			for(FNavigationOctree::TConstElementBoxIterator<FNavigationOctree::DefaultStackAllocator> It(*NavOctree, Generator->GetTotalBounds());
				It.HasPendingElements();
				It.Advance())
			{
				const FNavigationOctreeElement& Element = It.GetCurrentElement();
				const bool bExportGeometry = Element.Data->HasGeometry() && Element.ShouldUseGeometry(Generator->GetOwner()->GetConfig());

				if (bExportGeometry && Element.Data->CollisionData.Num())
				{
					// Why Can not resolved this constructor
					//const FRecastGeometryCache CachedGeometry(Element.Data->CollisionData.GetData());
					FRecastGeometryCache CachedGeometry;
					{
						const uint8* Memory = Element.Data->CollisionData.GetData();
						CachedGeometry.Header = *((FRecastGeometryCache::FHeader*)Memory);
						CachedGeometry.Verts = (float*)(Memory + sizeof(FRecastGeometryCache));
						CachedGeometry.Indices = (int32*)(Memory + sizeof(FRecastGeometryCache) + (sizeof(float) * CachedGeometry.Header.NumVerts * 3));
					}
				...
				else
				{
					...
					OffmeshLinks.Append(Element.Data->Modifiers.GetSimpleLinks());
					const FCustomLinkNavModifier* LinkModifier = Element.Data->Modifiers.GetCustomLinks().GetData();
					for (int32 i = 0; i < Element.Data->Modifiers.GetCustomLinks().Num(); i++, LinkModifier++)
					{
					...
					{
						if (OffmeshLinks[i].Links.Num() > 0)
						{
							const FNavigationLink& Link = OffmeshLinks[i].Links[0];
							FOffmeshLinkExportData ExportData;
							ExportData.Left = Unreal2RecastPoint(OffmeshLinks[i].LocalToWorld.TransformPosition(Link.Left));
							ExportData.Right = Unreal2RecastPoint(OffmeshLinks[i].LocalToWorld.TransformPosition(Link.Right));
							ExportData.Radius = Link.SnapRadius;
							ExportData.Direction = Link.Direction;
							ExportData.LinkId = Link.UserId;
							if (AActor** Actor = MySmartLinkProxy.FindByPredicate([&Link](const AActor* InActor){
								if (const ASmartNavLinkProxy* SmartNavLink = Cast<ASmartNavLinkProxy>(InActor))
								{
									return SmartNavLink->GetSmartLinkComp()->GetLinkId() == Link.UserId;
								}

								return false;
							}))
							{
								if (ASmartNavLinkProxy* SmartNavLink = Cast<ASmartNavLinkProxy>(*Actor))
								{
									ExportData.Duration = SmartNavLink->LinkTransitTime;
								}
							}
							OffmeshLinkDatas.Add(ExportData);
						}
					}
					#endif
				}
			...
			UWorld* NavigationWorld = Generator->GetWorld();
			...
			#if APPEND_OFFMESHLINK
			if (OffmeshLinkDatas.Num())
			{
				AdditionalData += "# Offmesh link export\n";
				for (int32 i = 0; i < OffmeshLinkDatas.Num(); i++)
				{
					const FOffmeshLinkExportData& ExportData = OffmeshLinkDatas[i];
					AdditionalData += FString::Printf(TEXT("c %f %f %f  %f %f %f %f %d %d %d %d %d\n"),
						ExportData.Left.X, ExportData.Left.Y, ExportData.Left.Z,
						ExportData.Right.X, ExportData.Right.Y, ExportData.Right.Z,
						ExportData.Radius, ExportData.Direction, ExportData.Area, ExportData.Flags,
						ExportData.LinkId, ExportData.Duration
					);
				}
				AdditionalData += "\n";
			}
			...
			FBox RCNavBounds = Unreal2RecastBox(Generator->GetTotalBounds());
			...
			check(CurrentGen);
			const FRecastBuildConfig& Config = CurrentGen->GetConfig();
			...
			#if 1 // use short file path 
				const FString FilePathName = FileName.EndsWith(".obj") ? FileName : FString::Printf(TEXT("%s.obj"), *FileName) ;
			#else
