...
void STrackNodeWidget::Construct(const FArguments& InArgs)
{
	...
	[
		...
		.BorderBackgroundColor(this, &STrackNodeWidget::GetNodeColor)
		.Content()
		[
			SNew(STextBlock)
			.Font(DrawFont)
...
FSlateColor STrackNodeWidget::GetNodeColor() const
{
...
FVector2D STrackNodeWidget::GetTrackNodeSize(const STrackWidget* Parent, const FGeometry& AllottedGeometry) const
{
	...
	{
		NodeWidth = Parent->DataToLocalX(Parent->ViewInputMin.Get() + NodeBlockLength.Get(), AllottedGeometry);
...
FReply STrackNodeWidget::OnDragDetected( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if (MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton))
	{
		if (bAllowDrag.Get())
		{
			...
			return FReply::Handled().BeginDragDrop(FLocalNodeDrageDropOp::New(SharedThis(this), ScreenCurPos, ScreenNodePosition));
		}
	}
...
int32 STrackWidget::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	FSlateDrawElement::MakeBox(
		...
		CustomLayerId++,
		MyGeometry, 
		FEditorStyle::GetBrush(TEXT("Graph.Node.NodeBackground")),
		ESlateDrawEffect::None,
		TrackColor.Get()
	);

	// Draw cursor
	if (CursorPosition.Get() >= 0.f)
	{
		float XPositiion = DataToLocalX(CursorPosition.Get(), AllottedGeometry);
